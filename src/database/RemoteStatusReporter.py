...
from src.database import API, Status
...
class RemoteStatusReporter(Status.OnChangeStatusReporter, threading.Thread):
        ...
        while True:
            ...
            try:
                ...
                for res in socket.getaddrinfo(None, 
                    ...
                    logger.info("Remote status reporter listening on %s"% str(sa))
            ...
            try:
                ...
                try:
                    ...
                    while float(self._requests[0][TIME]) <= now:
                        ...
                        if not elem:
                            logger.error("Unknown parameter should be reported: %s" % param)
                            continue
                ...
                logger.exception("Error status reporting")
    ...
    def _split_param(self, param):
        ...
        while True:
            ...
            if param[:pos] in self.status_holders.keys():
        ...
        try:
            for line in data.split("\n"):
                ...
                try:
                    ...
                    logger.error("Bad request: '%s'"%line)
                    continue
                ...
                if command[0] == "SUBSCRIBE":
                    ...
                    if param.lower() == "all":
                        for holder in self.status_holders.keys():
                            for name in self.status_holders[holder].keys():
                                if type == "onchange":
                    ...
                    elem = self.status_holders[holder][name]
                    if not elem:
                        logger.error("Ignoring request for unknown parameter '%s'"%param)
                        continue
                    ...
                    if type == "once":
                        ...
                        logger.error("Bad request type '%s', expected one of 'once', 'onchange' or 'periodic'"%type)
                        continue
                    
                    logger.debug("Got subscribe '%s' on %s"% (type, param))
                    ...
                    elem = self.status_holders[holder][name]
                    if not elem:
                        logger.error("Ignoring unsubscribe for unknown parameter '%s'"%param)
                        continue

                    logger.debug("Got unsubscribe %s"% param)
                    ...
                    for holder in self.status_holders.values():
                        for name in holder.list_status_elements():
                            elem = holder[name]
                            self._send_update(elem, socket)
                    logger.debug("LIST request OK")
                        
        except:
            logger.exception("Processing request from " + str(self._addresses[socket]))
    ...
    def _send_update(self, element, socket):
        ...
        try:
            ...
            logger.exception("Sending update on '%s to %s"% \
                               (element.get_name(),
